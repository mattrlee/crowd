...
$existingcourseselector = new course_existing_selector('removeselect', array('crowdid'=>$crowd->id, 'accesscontext'=>$context));

// Process incoming user assignments to the cohort

if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    $coursestoassign = $potentialcourseselector->get_selected_rows();
    if (!empty($coursestoassign)) {

        foreach ($coursestoassign as $course) {
            crowd_add_course($crowd->id, $course->id);
        }

        $potentialcourseselector->invalidate_selected_rows();
        $existingcourseselector->invalidate_selected_rows();
    }
}

// Process removing user assignments to the cohort
if (optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    $coursestoremove = $existingcourseselector->get_selected_rows();
    if (!empty($coursestoremove)) {
        foreach ($coursestoremove as $removecourse) {
            crowd_remove_course($crowd->id, $removecourse->id);
        }
        $potentialcourseselector->invalidate_selected_rows();
        $existingcourseselector->invalidate_selected_rows();
    }
