...
if ($_SERVER['PHP_SELF'] == parse_url($CFG->wwwroot, PHP_URL_PATH).'/index.php') {
	...
	if ($visibility) {
		...
		$PAGE->requires->js('/local/crowd/jquery.cookie.js', true); 
...
?><div class="crowd_category_box tooltip <?php echo $course['access'] > 0 ? 'crowd_visible_course' : 'crowd_hidden crowd_grey' ?>" <?php echo $course['access'] > 0 ? 'cid="'.$course['id'].'"' :'' ?> title="<?php echo $course['description']?>">
<div class="crowd_category_image_box"><?php echo $course['img']?></div>
<div class="crowd_empty_box"></div>
